...
router.post("/api/bookings", authenticateToken, requireRole('student'), async (req, res) => {
    try {
        ...
        const [spRows] = await pool.query(
            "SELECT teacher_id, sessions_remaining FROM student_packages WHERE id = ? AND company_id = ?",
            [student_package_id, companyId]
        );
        if (spRows.length === 0) return res.status(404).json({ message: "Package not found" });
