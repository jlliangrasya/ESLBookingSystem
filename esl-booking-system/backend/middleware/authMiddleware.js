...
const authenticateToken = async (req, res, next) => {
    ...
    try {
        ...
        try {
            ...
            // DB query failed (cold start, timeout) — don't log user out, just proceed
            // The JWT is valid, so let the request through
        }

        next();
    } catch (error) {
        // Only JWT verification failure triggers 401 (actual invalid/expired token)
